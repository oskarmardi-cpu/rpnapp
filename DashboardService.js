...
const DashboardService = (() => {
  ...
  /*=======================================================
    LIVE POPULATION KPI — LOCKED BASELINE
    1. Only the first UNIT population table is counted.
    2. Reading stops at the next No + SN table header.
    3. Battery / Charger / Spare rows are excluded.
    4. TOTAL UNIT USED = actual unit rows with SN.
    5. RFU = actual STATUS EPICOR RFU.
    6. NON RFU = actual STATUS EPICOR NON RFU.
    7. No TOTAL - RFU mathematical fallback.
  =======================================================*/
  ...
  function movementHeader_(row) {
    return {period:findColumn_(row,["PERIODE","PERIOD","BULAN","MONTH"]),in:findColumn_(row,["IN","UNIT IN","MASUK"]),out:findColumn_(row,["OUT","UNIT OUT","KELUAR"]),balance:findColumn_(row,["BALANCE","SALDO","CURRENT BALANCE"])};
  }
  ...
  function parseDirectMovement_(values) {
    for (let r=0;r<values.length;r++) {
      ...
      for(let i=r+1;i<values.length;i++){
        ...
        rows.push({period:period,in:number_(source[h.in]),out:number_(source[h.out]),balance:number_(source[h.balance])});
      }
  ...
  function parseMatrixMovement_(values) {
    ...
    for(let r=0;r<values.length;r++){
      const row=values[r]||[],label=upper_(row[0]);
      const hasMonth=row.some(function(v){return /^(JANUARY|FEBRUARY|MARCH|APRIL|MAY|JUNE|JULY|AUGUST|SEPTEMBER|OCTOBER|NOVEMBER|DECEMBER|JAN|FEB|MAR|APR|MEI|JUN|JUL|AGS|AUG|SEP|OKT|OCT|NOV|DES|DEC)([- ]?\d{2,4})?$/i.test(text_(v));});
      if((label==="KETERANGAN"||label==="PERIODE")&&hasMonth){headerRow=r;periodCols=row.map(function(v,idx){return{label:text_(v),idx:idx};}).filter(function(x){return x.label&&x.idx>0&&x.label!=="TOTAL";});break;}
    }
  ...
  function getSummaryMovement_(){
    ...
    const values=ws.getDataRange().getValues(),rows=parseDirectMovement_(values),movement=rows.length?rows:parseMatrixMovement_(values);
    if(!movement.length)throw new Error("Struktur data Movement pada sheet SUMMARY tidak ditemukan. Pastikan tersedia kolom PERIODE, IN, OUT, BALANCE.");
    const totalIn=movement.reduce(function(sum,item){return sum+number_(item.in);},0),totalOut=movement.reduce(function(sum,item){return sum+number_(item.out);},0),currentBalance=movement.reduce(function(last,item){return item.balance!==0?item.balance:last;},movement[movement.length-1].balance);
  ...
  function buildChart_(kpi,monthly,movement){return{status:{labels:["RFU","NON RFU","Loan"],datasets:[{data:[kpi.rfu,kpi.nonRfu,kpi.loanPart]}]},monthly:{labels:movement.rows.map(function(item){return item.period;}),datasets:[{label:"IN (Unit Masuk)",data:movement.rows.map(function(item){return item.in;}),backgroundColor:"#2f82ee",borderColor:"#2f82ee",borderWidth:1},{label:"OUT (Unit Keluar)",data:movement.rows.map(function(item){return item.out;}),backgroundColor:"#203b6f",borderColor:"#203b6f",borderWidth:1},{label:"BALANCE (Saldo Akhir)",data:movement.rows.map(function(item){return item.balance;),type:"line",borderColor:"#ef4444",backgroundColor:"#ef4444",borderWidth:2,tension:.25,fill:false,pointRadius:3,pointHoverRadius:5}]}};}

  function getDashboard(){
    try{
      Auth.check();
      const user=Auth.getUser()||{},populationKpi=getPopulationKPI_(),movement=getMovement_(),loanPart=getLoanPart_(),summaryMovement=getSummaryMovement_();
      const dashboard={user:{id:user.USER_ID||"",username:user.USERNAME||"",name:user.FULLNAME||"Administrator",role:user.ROLE||"",branch:user.BRANCH||""},kpi:{totalUnit:populationKpi.totalUnit,unitDitarik:getUnitDitarik_(movement),unitDikirim:getUnitDikirim_(movement),rfu:populationKpi.rfu,nonRfu:populationKpi.nonRfu,loanPart:loanPart},population:populationKpi,movement:summaryMovement,chart:buildChart_(populationKpi,getMonthly_(movement),summaryMovement),activity:getActivity_(),report:[]};
