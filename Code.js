/* RPN MANAGEMENT SYSTEM - DIRECT ENTRY POINTS */
function dashboardGet(){try{Auth.check();return DashboardService.getDashboard();}catch(err){Logger.error("Code.dashboardGet",err);return Response.error(err);}}
...
function authLogin(username,password){try{return Auth.login(String(username||"").trim(),String(password||""));}catch(err){Logger.error("Code.authLogin",err);return Response.error(err);}}
...
function renderPopulationRegional(branch){Auth.check();return HtmlService.createHtmlOutputFromFile("27_PopulasiRegional").getContent();}
