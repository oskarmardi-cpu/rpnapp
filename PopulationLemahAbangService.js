/*=========================================================
 ...
 FIX #2 — PENGIRIMAN UNIT + COMPONENT POPULATION SYNC

 Scope FIX #2:
 - Pengiriman Unit branch Lemah Abang is written to population movement state.
 - Unit, Battery, Battery Spare, Charger and Trolley are removed from
   POPULASI UNIT USED LEMAH ABANG by their respective serial numbers.
 - Existing read/manual-edit behavior is preserved.
 - No other workflow is changed.
=========================================================*/
const PopulationLemahAbangService = (() => {
  ...
  function headerMap_(headers) {
    ...
    headers.forEach((h,i) => {
      ...
      Object.keys(ALIAS).forEach(k => {
        if (ALIAS[k].some(a => norm_(a) === n) && map[k] === undefined) map[k] = i;
      });
    });
    return map;
  }
  ...
  function sourceFormat_(ws,values) {
    const maxCol = Math.max(ws.getLastColumn(),1);
    const maxRow = Math.max(ws.getLastRow(),1);
    let headerRow = -1;
    for (let r=0;r<values.length;r++) {
      if (norm_(values[r][0]) === "NO") { headerRow=r+1; break; }
    }
  ...
  function read() {
    Auth.check();
    const x=matrix_();
    const values=x.ws.getDataRange().getValues();
    ...
    return Response.success({
      ...
      HEADERS:x.headers,
      ROWS:rows,
      EDITABLE:user.ROLE==="ADMIN"||user.ROLE==="SUPERVISOR",
      EDIT_URL:EDIT_URL,
      SOURCE_FORMAT:sourceFormat_(x.ws,values)
    });
  }

  function set_(ws,row,col,value) {
    if (col!==undefined) ws.getRange(row,col+1).setValue(value);
  }

  function findBySerial_(x,serial) {
    const target=norm_(serial);
    if (!target || x.map.SERIAL===undefined) return -1;
    for (let i=0;i<x.rows.length;i++) {
      if (norm_(x.rows[i][x.map.SERIAL])===target) return i+2;
    }
    return -1;
  }

  function ensureAssetRow_(x,assetType,serial,unitSerial) {
    if (!serial || x.map.SERIAL===undefined) return;
    const ws=x.ws;
    let row=findBySerial_(x,serial);
    if (row<0) {
      row=Math.max(ws.getLastRow()+1,2);
      set_(ws,row,x.map.SERIAL,serial);
  ...
  /*=======================================================
    FIX #2 — REMOVE ASSET FROM USED POPULATION
  =======================================================*/

  function removePopulationAsset_(x,serial,assetType) {
    const sn=norm_(serial);
    if (!sn || x.map.SERIAL===undefined) return {UPDATED:false,ASSET_TYPE:assetType,REASON:"SERIAL_EMPTY"};

    const row=findBySerial_(x,serial);
    if (row<0) return {UPDATED:false,ASSET_TYPE:assetType,SERIAL_NUMBER:serial,REASON:"SERIAL_NOT_FOUND"};

    const ws=x.ws;

    /*
     * Do not delete physical rows. The population sheet is an operational
     * source and its row history must remain stable. Mark the asset as OUT
     * instead, so the live population parser can exclude it in the next step.
     */
    if (x.map.STATUS!==undefined) set_(ws,row,x.map.STATUS,"PENGIRIMAN");
    if (x.map.BRANCH!==undefined) set_(ws,row,x.map.BRANCH,"LEMAH ABANG - OUT");
    if (x.map.ACTION!==undefined) set_(ws,row,x.map.ACTION,"PENGIRIMAN UNIT");
    if (x.map.UPDATED_AT!==undefined) set_(ws,row,x.map.UPDATED_AT,new Date());

    return {UPDATED:true,ASSET_TYPE:assetType,SERIAL_NUMBER:serial,ROW_NUMBER:row};
  }

  function removePopulationByShipping_(data) {
    if (!data || norm_(data.BRANCH)!=="LEMAH ABANG") {
      return {UPDATED:false,REASON:"BRANCH_NOT_LEMAH_ABANG",ASSETS:[]};
    }

    const category=norm_(data.CATEGORY);
    if (category!=="PENGIRIMAN UNIT") {
      return {UPDATED:false,REASON:"CATEGORY_NOT_PENGIRIMAN_UNIT",ASSETS:[]};
    }
    ...
    const candidates=[
      {type:"UNIT",serial:data.SN_UNIT||data.SERIAL_NUMBER},
      {type:"BATTERY",serial:data.BATTERY_IN||data.BATTERY_SN},
      {type:"BATTERY_SPARE",serial:data.BATTERY_SPARE_SN||data.BATTERY_SPARE||data.SN_BATTERY_SPARE},
      {type:"CHARGER",serial:data.CHARGER_SN||data.CHARGER_IN},
      {type:"TROLLEY",serial:data.TROLLEY_SN||data.TROLLEY||data.SN_TROLLEY}
    ];

    candidates.forEach(item=>{
      const serial=String(item.serial==null?"":item.serial).trim();
      if(!serial || seen[norm_(serial)]) return;
      seen[norm_(serial)]=true;
      assets.push(removePopulationAsset_(x,serial,item.type));
    });

    SpreadsheetApp.flush();

    return {
      UPDATED:assets.some(a=>a.UPDATED),
      CATEGORY:category,
      BRANCH:data.BRANCH,
      ASSETS:assets,
      UPDATED_COUNT:assets.filter(a=>a.UPDATED).length,
      NOT_FOUND:assets.filter(a=>a.REASON==="SERIAL_NOT_FOUND").map(a=>({ASSET_TYPE:a.ASSET_TYPE,SERIAL_NUMBER:a.SERIAL_NUMBER}))
    };
  }
  ...
  function updateFromRefurbish(data) {
    ...
    if(row<0&&unitSerial&&x.map.SERIAL!==undefined){
      row=Math.max(ws.getLastRow()+1,2);
      set_(ws,row,x.map.SERIAL,unitSerial);
    }
    ...
    if(category==="PENGIRIMAN UNIT"){
      const battery=data.BATTERY_IN||"",charger=data.CHARGER_SN||"";
    ...
    /* FIX #2: only shipping flow triggers removal from live LA population. */
    const populationSync=category==="PENGIRIMAN UNIT"
      ? removePopulationByShipping_(data)
      : {UPDATED:false,REASON:"NOT_SHIPPING",ASSETS:[]};
